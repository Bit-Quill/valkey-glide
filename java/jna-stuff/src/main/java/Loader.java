...
public class Loader {
  ...
  public static void main(String[] args) {
    ...
    var lib = (RustLib) Native.load("javababushka", RustLib.class);
    ...
    if (con.value_type == ResultType.Ok.id) {
      ...
      System.out.printf(
          "Conn failed: Res = %s, str = %s, err = %s%n",
          ResultType.of(con.value_type), con.string, con.error);
    }
    ...
    if (set.value_type == ResultType.Ok.id) {
      ...
      System.out.printf(
          "Set failed: Res = %s, str = %s, err = %s%n",
          ResultType.of(set.value_type), set.string, set.error);
    }

    System.out.println("=======\nBefore get1");
    var get1 = lib.get0(bab, "kkkey");
    System.out.println("After get1");
    if (get1.value_type == ResultType.Str.id) {
      System.out.printf("Get1 existing ok: %s%n", get1.string);
    } else {
      System.out.printf(
          "Set failed: Res = %s, str = %s, err = %s%n",
          ResultType.of(get1.value_type), get1.string, get1.error);
    }

    System.out.println("=======\nBefore get2");
    var get2 = lib.get0(bab, "kkk");
    System.out.println("After get2");
    if (get2.value_type == ResultType.Str.id) {
      System.out.printf("Get2 not existing ok: %s%n", get2.string);
    } else {
      System.out.printf(
          "Set failed: Res = %s, str = %s, err = %s%n",
          ResultType.of(get2.value_type), get2.string, get2.error);
    }
