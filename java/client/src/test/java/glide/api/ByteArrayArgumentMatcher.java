 ...
 * <p>
 * It is used in Mockito verifications to assert that a method call was made
 * with a specific list of byte arrays as arguments.
 */
public class ByteArrayArgumentMatcher implements ArgumentMatcher<List<byte[]>> {
     ...
     * @return boolean - true if the provided list of byte arrays matches the stored arguments,
     *                   false otherwise.
     */
    @Override
    public boolean matches(List<byte[]> t) {
        ...
        for (int index = 0; index < length; index++) {
            if (!(new String(t.get(index)).equals(new String(arguments.get(index)))))
                return false;
        }
        return true;
    }
