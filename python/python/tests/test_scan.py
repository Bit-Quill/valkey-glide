...
async def is_cluster_ready(client: GlideClusterClient, count: int) -> bool:
    ...
    # we need to make sure that the inner core refresh slots so we make sure we accumulate 60 seconds
    await asyncio.sleep(60 - (asyncio.get_event_loop().time() - start_time))
...
class TestScan:
    ...
    async def test_cluster_scan_non_covered_slots(
        ...
        key = get_random_string(10)
        for i in range(1000):
            result = await glide_client_scoped.set(f"{key}:{i}", "value")
            assert result == "OK"
        cursor = ClusterScanCursor()
